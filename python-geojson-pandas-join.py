...
def join(geojson, excel_dataframe, key_geojson, key_pandas, output, missing_columns=0, missing_rows=0):
...
def join_df(geojson, dataframe, key_geojson, key_pandas, output, missing_columns=0, missing_rows=0):
    """ this library add 'columns' to geojson from 'pandas'
    :param geojson: geojson build with qgis (url)
    :param dataframe: simple excel without multindex and monodimensional (url)
    :param key_geojson: key column
    :param key_pandas: key column
    :param output: url
    :param missing_columns: number of cell with NAN value for row to drop
    ...
    return "executed " + geojson + " pandas dataframe"
